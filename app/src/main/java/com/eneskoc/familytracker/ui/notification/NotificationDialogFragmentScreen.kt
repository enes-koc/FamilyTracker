...
class NotificationDialogFragmentScreen : DialogFragment(), NotificationAdapterOnItemClickListener {
    ...
    override fun onAcceptButtonClicked(user: UserDataHolder) {
        ...
        user.uid?.let { authViewModel.acceptFollowRequest(it) }
        viewLifecycleOwner.lifecycleScope.launch {
            authViewModel.acceptFollowRequestFlow.collect { resource ->
                when (resource) {
                    is Resource.Success -> {
                        listenFollowRequest()
                    }
    ...
    override fun onRejectButtonClicked(user: UserDataHolder) {
        user.uid?.let { authViewModel.rejectFollowRequest(it) }
        viewLifecycleOwner.lifecycleScope.launch {
            authViewModel.rejectFollowRequestFlow.collect { resource ->
                when (resource) {
                    is Resource.Success -> {
                        listenFollowRequest()
                    }
